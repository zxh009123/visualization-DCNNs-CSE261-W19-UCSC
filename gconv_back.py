...
class res18GConv(nn.Module):
    def __init__(self, num_class = 1000):
        ...
        self.expand_InsNorm0 = nn.BatchNorm2d(num_class, affine=True)
        ...
        self.conv2dT3 = nn.ConvTranspose2d(num_class, num_class, kernel_size=4, stride=2, padding=1, bias=False)
        self.InsNorm3 = nn.BatchNorm2d(num_class, affine=True)
        self.relu3 = nn.ReLU(inplace=True)
        self.drop3 = nn.Dropout2d(p = 0.3)

        self.conv2dT4 = nn.ConvTranspose2d(num_class, num_class, kernel_size=4, stride=2, padding=1, bias=False, groups=num_class)
        self.InsNorm4 = nn.BatchNorm2d(num_class, affine=True)
    ...
    def forward(self,x):
        ...
        upSample3 = self.conv2dT2(upSample2_drop)
        upSample3_norm = self.InsNorm2(upSample3)
        upSample3_relu = self.relu2(upSample3_norm)
        upSample3_drop = self.drop2(upSample3_relu)

        upSample4 = self.conv2dT3(upSample3_drop)
        upSample4_norm = self.InsNorm3(upSample4)
        upSample4_relu = self.relu3(upSample4_norm)

        result = self.avgPool(upSample4_relu)
        result = result.view(result.size(0), -1)

        return result, upSample4_relu, upSample3_drop
